...
export async function extractSingleEmbedding(source: Source): Promise<{
  ...
  const result = await faceapi
    .detectSingleFace(source as faceapi.TNetInput, new faceapi.TinyFaceDetectorOptions({ inputSize: 608, scoreThreshold: 0.4 }))
    .withFaceLandmarks()
    .withFaceDescriptor()
...
export async function extractAllEmbeddings(source: Source): Promise<Array<{
  ...
  const results = await faceapi
    .detectAllFaces(source as faceapi.TNetInput, new faceapi.TinyFaceDetectorOptions({ inputSize: 608, scoreThreshold: 0.4 }))
    .withFaceLandmarks()
    .withFaceDescriptors()
